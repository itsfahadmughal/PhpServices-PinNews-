<?php
    if(file_exists("config_file.php") && is_readable("config_file.php") && include("config_file.php")) 
    {
		...
		$sql = "SELECT DISTINCT title, image, content, likes, share, date_entry, category, news_channel,view FROM all_news WHERE date_entry=(SELECT MAX(date_entry) FROM all_news) AND title != 'Image News!' AND category IN ('".implode("','",array_map(null, $temp))."') ";
		...
		// For old ones
		$sql2 = "SELECT DISTINCT title, image, content, likes, share, date_entry, category, news_channel,view FROM all_news WHERE title<>'Image News!' AND date_entry != '$date' AND category IN ('".implode("','",array_map(null, $temp))."') ";
